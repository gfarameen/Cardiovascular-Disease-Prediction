...
def preprocess_data(data):
    ...
    data["Smoking"] = data["Smoking"].replace({"Current": 1, "Never": 0, "Former": 2})
    data["Alcohol Intake"] = data["Alcohol Intake"].replace(
        {"Heavy": 1, "NaN": np.nan, "Moderate": 2}
    )
    ...
    data["Exercise Induced Angina"] = data["Exercise Induced Angina"].replace(
        {"Yes": 1, "No": 0}
    )
    ...
    data["Mean Arterial Pressure"] = (
        data["Blood Pressure"] + (data["Blood Pressure"] - data["Blood Pressure"]) / 3
    )
    data["Risk_Score"] = (
        0.3 * data["Age"]
...
def predict():
    ...
    return render_template(
        "result.html",
        prediction_text=f'Heart Disease Prediction: {"Yes" if output == 1 else "No"}',
    )
